...
public class BackEndHandler extends Thread             
{
	...
	public boolean chooseMasterFile()
	{
		...
		else if(!(fileChooser.getSelectedFile().getName().endsWith(".mp3")||fileChooser.getSelectedFile().getName().endsWith(".wav")||fileChooser.getSelectedFile().getName().endsWith(".ram")||fileChooser.getSelectedFile().getName().endsWith(".wma")))
		{
			JOptionPane.showMessageDialog(null,"Please Select the File which has .mp3 or .wav or .ram or .wma Extention");
			return false;
		}
			else
	...
	public boolean chooseOutputFile()
	{
		...
		do
		{
			...
			result= fileChooser.showDialog(null, "Select Output file");
				if(!(fileChooser.getSelectedFile().getName().endsWith(".mp3")||fileChooser.getSelectedFile().getName().endsWith(".wav")||fileChooser.getSelectedFile().getName().endsWith(".ram")||fileChooser.getSelectedFile().getName().endsWith(".wma")))
		{
		//	JOptionPane.showMessageDialog(null,"Please Select the File which has .mp3 or .wav or .ram or .wma");
			break;
		}
			if(result== fileChooser.APPROVE_OPTION)
			{
		...
		if(result== fileChooser.CANCEL_OPTION)	return false;
			if(!(fileChooser.getSelectedFile().getName().endsWith(".mp3")||fileChooser.getSelectedFile().getName().endsWith(".wav")||fileChooser.getSelectedFile().getName().endsWith(".ram")||fileChooser.getSelectedFile().getName().endsWith(".wma")))
		{
			JOptionPane.showMessageDialog(null,"Please Select the File which has .mp3 or .wav or .ram or .wma Extension");
			return false;
		}
